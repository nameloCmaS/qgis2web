...
from qgis2web.utils import safeName, scaleToZoom
...
def crsScript(crsAuthId, crsProj4):
    crs = """
        var crs = new L.Proj.CRS('{crsAuthId}', '{crsProj4}', {{
            resolutions: [2800, 1400, 700, 350, """.format(
        crsAuthId=crsAuthId, crsProj4=crsProj4
    )
...
def iconLegend(symbol, catr, outputProjectFileName, layerName, catLegend, cnt):
    ...
    legendIcon = QgsSymbolLayerUtils.symbolPreviewPixmap(
        symbol, QSize(iconSize, iconSize)
    )
    safeLabel = re.sub(r"[\W_]+", "", catr.label()) + str(cnt)
    legendIcon.save(
        os.path.join(
            outputProjectFileName, "legend", layerName + "_" + safeLabel + ".png"
        )
    )
...
def wmsScript(layer, safeLayerName, useWMS, useWMTS, identify, minZoom, maxZoom, count):
    ...
    zIndex = count + 400
    if attrText != "":
        attr = '<a href="%s">%s</a>' % (attrUrl, attrText)
    wms = """
...
def rasterScript(layer, safeLayerName, zIndex):
    ...
    raster = """
        ...
        safeLayerName=safeLayerName, zIndex=zIndex, out_raster=out_raster, bounds=bounds
    )
    raster += "new L.imageOverlay(img_"
    raster += """{sln},
                                              img_bounds_{sln},
                                              {{pane: 'pane_{sln}'}});
        bounds_group.addLayer(layer_{sln});""".format(sln=safeLayerName)
    return raster


def titleSubScript(title, pos):
    if pos == "upper right":
        positionOpt = "{'position':'topright'}"
    if pos == "lower right":
        positionOpt = "{'position':'bottomright'}"
    if pos == "lower left":
        positionOpt = "{'position':'bottomleft'}"
    if pos == "upper left":
        positionOpt = "{'position':'topleft'}"
    titleSub = ""
    if pos != "None":
        titleSub = (
            """
...
def addLayersList(
    baseMap, matchCRS, layer_list, groups, collapsedGroup, cluster, legends, expanded
):
    controlStart = """
        var overlaysTree = [
    """
    ...
    for i, clustered in zip(reversed(layer_list), reversed(cluster)):
                ...
                for group_name, group_layers in groups.items():
                    if i in group_layers:
                        ...
                        if group_name not in created_groups:
                            ...
                            collapsed = (
                                "collapsed: true,"
                                if collapsedGroup[collapsedGroupCount]
                                else ""
                            )
        ...
        {label: '<b>"""
            ...
            for group_name in created_groups:
                if group_name not in closed_groups and len(
                    created_groups[group_name]
                ) == len(groups[group_name]):
                    layersList += (
                        "]},"  # Chiude il gruppo se tutti i layer sono stati aggiunti
                    )
            ...
            QgsMessageLog.logMessage(
                traceback.format_exc(), "qgis2web", level=Qgis.Critical
            )
    ...
    if filterNum != 0:
        ...
        for item in range(0, filterNum):
            ...
            {nameS}_options_str  += '<option value="{e}">{e}</option>';
                        """.format(e=safeEntry, name=itemName, nameS=safeName(itemName))
                endHTML += """
