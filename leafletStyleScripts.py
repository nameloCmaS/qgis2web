...
def getLayerStyle(
    layer, sln, interactivity, markerFolder, outputProjectFilename, useShapes, feedback
):
    markerType = None
    useMapUnits = False
    ...
    if isinstance(renderer, QgsNullSymbolRenderer):
        ...
        for sl in range(slCount):
            (styleCode, markerType, useMapUnits, pattern) = getSymbolAsStyle(
                symbol,
                markerFolder,
                layer_alpha,
                interactivity,
        ...
        function style_%s_%s(feature) {
            switch(String(feature.properties['%s'])) {""" % (
                sln,
                sl,
                classAttr.replace("'", "\\'"),
            )
...
def getSymbolAsStyle(
    ...
    if isinstance(sl, QgsSimpleMarkerSymbolLayer):
        ...
        strokeStyle, useMapUnits = getStrokeStyle(
            borderColor,
            borderStyle,
            borderWidth,
            line_units,
...
def getMarker(
    ...
    return (
        """
                %s
                radius: %s,%s%s"""
        % (markerShape, size, strokeStyle, getFillStyle(color, props)),
        useMapUnits,
    )
...
def getStrokeStyle(
    color, dashed, width, units, linecap, linejoin, useMapUnits, feedback, props
):
    if dashed != "no":
