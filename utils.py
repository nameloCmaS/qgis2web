...
MB_TYPE_MAP = {"Point": "symbol", "LineString": "line", "Polygon": "fill"}
...
def writeTmpLayer(layer, restrictToExtent, iface, extent):
    ...
    for field in usedFields:
        fieldIndex = layer.fields().indexFromName(layer.fields().field(field).name())
        editorWidget = layer.editorWidgetSetup(fieldIndex).type()
    ...
    if restrictToExtent and extent == "Canvas extent":
        ...
        try:
            transform = QgsCoordinateTransform(
                canvasCRS, layerCRS, QgsProject.instance()
            )
        except Exception:
    ...
    for feature in features:
        ...
        if writer.hasErrors() != QgsVectorFileWriter.NoError:
            QgsMessageLog.logMessage(
                "Error when creating tempory layer: {}".format(writer.lastError()),
                "qgis2web",
                level=Qgis.Critical,
            )
    del writer
...
def exportVector(
    layer, sln, layersFolder, restrictToExtent, iface, extent, precision, crs, minify
):
    canvas = iface.mapCanvas()
    ...
    if hasattr(QgsVectorFileWriter, "writeAsVectorFormatV3"):
        ...
        result = QgsVectorFileWriter.writeAsVectorFormatV3(
            cleanLayer, tmpPath, QgsProject.instance().transformContext(), save_options
        )
    else:
        # Use writeAsVectorFormat for QGIS versions < 3.10.3 for backwards compatibility
        result = QgsVectorFileWriter.writeAsVectorFormat(
            cleanLayer, tmpPath, "utf-8", crs, "GeoJson", 0, layerOptions=options
        )
    if result:
...
def exportRaster(layer, count, layersFolder, feedback, iface, matchCRS):
    ...
    file_writer.writeRaster(
        pipe, piped_height, -1, piped_extent, piped_crs, file_writer_transformContext
    )

    # Export layer as PNG
    out_raster = os.path.join(
        layersFolder, safeName(layer.name()) + "_" + str(count) + ".png"
    )
    ...
    if not (matchCRS and layer.crs() == projectCRS):
        ...
        # Reproject in 3857
        piped_3857 = os.path.join(tempfile.gettempdir(), name_ts + "_piped_3857.tif")
...
def is25d(layer, canvas, restrictToExtent, extent):
    ...
    for sym in symbols:
        try:
            ...
            return False
        if isinstance(sl1, QgsGeometryGeneratorSymbolLayer) and isinstance(
            sl2, QgsGeometryGeneratorSymbolLayer
        ):
            return True
    return False
...
def safeName(name):
    ...
    validChr = "_0123456789abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ"
...
def replaceInTemplate(template, values):
    path = os.path.join(
        QgsApplication.qgisSettingsDirPath(), "qgis2web", "templates", template
    )
    with open(path) as f:
        lines = f.readlines()
    s = "".join(lines)
    ...
    for feature in layer.getFeatures(fr):
        ...
        if not os.path.isabs(source_file_name):
            ...
            source_file_name = os.path.join(
                os.path.dirname(prj_fname), source_file_name
            )

        photo_file_name = re.sub(r"[\\/:]", "_", photo_file_name).strip()
        photo_file_name = os.path.join(
            os.path.dirname(layerFileName), "..", "images", photo_file_name
        )
...
def boilType(fieldType):
    ...
    if fieldType.lower() in [
        ...
        fType = "int"
    if fieldType.lower() in ["char", "string", "text", "varchar", "nchar", "nvarchar"]:
        fType = "str"
...
def returnFilterValues(layer_list, fieldName, fieldType):
    ...
    if fieldType == "int":
        cleanFilterValues = [
            min(filterValues) if min(filterValues) >= 0 else 0,
            max(filterValues) if max(filterValues) >= 0 else 0,
        ]
    ...
    if fieldType in ["date", "time", "real", "datetime"]:
        cleanFilterValues = [min(filter(None, filterValues)), max(filterValues)]
        if cleanFilterValues[0] == cleanFilterValues[1]:
            if fieldType == "real":
